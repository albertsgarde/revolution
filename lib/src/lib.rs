...
use crate::recipes::{AssemblerRecipe, FurnaceRecipe};

pub fn start() -> (Tick, Resource<{ ResourceType::Iron }>) {
    (Tick::start(), Resource::<{ ResourceType::Iron }> { amount: 10 })
}

pub fn win(tick: Tick, _points: Bundle<{ ResourceType::Point }, 10>) -> ! {
